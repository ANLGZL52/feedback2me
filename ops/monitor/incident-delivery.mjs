  ...
  for (const a of actions) {
    ...
    try {
      ...
      if (a.action === 'CREATE' || (!inc.issueNumber && (a.action === 'UPDATE' || a.action === 'REOPEN' || a.action === 'RESOLVE'))) {
        ...
        if (existing) { inc.issueNumber = existing; if (a.action === 'CREATE') action = { ...a, action: 'UPDATE', reason: 'reused_existing_issue' }; }
      ...
      events.push(ev(map[action.action] || 'incident.delivery.updated', a, inc, { result: 'ok' }));
...
function main() {
  ...
  const write = (extra = {}) => {
    writeFileSync(stateP, JSON.stringify({ generatedAt: new Date(now).toISOString(), incidents }, null, 2));
    writeFileSync(join(dir, 'incident-actions.json'), JSON.stringify({ generatedAt: new Date(now).toISOString(), mode, plan, ...extra }, null, 2));
  ...
  let events = [], failures = 0, deliveryStatus = 'LIVE_OK';
  try {
  ...
  write({ deliveryStatus, deliveryEvents: events, deliveryFailures: failures });
